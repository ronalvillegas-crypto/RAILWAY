...
class DetectorMovimientos:
    ...
    def __init__(self):
        # UMBRALES POR TIPO DE ACTIVO (en porcentaje) - SIN CRYPTO
        self.umbrales = {
            'forex': 0.003,      # 0.3%
            'commodities': 0.005, # 0.5%  
            'indices': 0.008,     # 0.8%
            # 'crypto': 0.020     # ELIMINADO
        }
        ...
        logger.info("✅ Detector de Movimientos inicializado (SIN CRYPTO)")
    ...
    def clasificar_tipo_activo(self, par):
        ...
        # Metales preciosos
        if any(metal in par_upper for metal in ['XAU', 'XAG', 'XPT', 'XPD']):
            return 'commodities'
        # Energía
        elif any(energia in par_upper for energia in ['OIL', 'NGAS']):
            return 'commodities'
        # Índices
        elif any(indice in par_upper for indice in ['SPX', 'NAS', 'DJI', 'GER', 'UK', 'JPN']):
            return 'indices'
        # Metales industriales
        elif any(metal in par_upper for metal in ['COPPER', 'ALUMINUM', 'NICKEL']):
            return 'commodities'
        # Forex (por defecto)
        else:
            return 'forex'
